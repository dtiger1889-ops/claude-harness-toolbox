...
| `hooks/workflow_gate.ps1` | PreToolUse **workflow runaway guard**: block-once gate on the first `Workflow` tool call — prints the abort-on-first-rate-limit + explicit-caps rules at the exact moment they matter, then lets the re-issued call through. The Workflow tool doesn't stop itself on session/rate limits; a prose rule read hours earlier loses to a hook that fires at the call. Also the template for any "when you use tool X, remember Y" rule. |
