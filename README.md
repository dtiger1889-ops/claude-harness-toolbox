...
| Skill | What it is |
|---|---|
| `checkpoint` | `/checkpoint`: rewrites CHECKPOINT.md in place to the schema, enforces the 120-line/~30KB caps, and archives overflow via `archive-changelog.ps1`, a byte-faithful mover script (the model picks which lines to evict; the script moves bytes the model never retypes). One call to `finish-checkpoint.ps1` closes a session: it sorts the Open-threads tags, stamps `Last updated:` with real UTC, auto-archives changelog overflow, and reports both caps — replacing three hand-run measurement round-trips and the hand-typed timestamp. `sort_open_threads.ps1` is the sorter it calls. Before the finisher, `verify_checkpoint_claims.ps1` checks the file's own claims: a dead path or a stale Open-threads bullet nobody re-confirmed fails the close (exit 2) instead of printing a warning the model can ignore. **Requires PowerShell 5.1+ (Windows) or `pwsh` 7+ (macOS/Linux);** the SKILL.md's last section says what to do by hand when the runtime has no shell. |
